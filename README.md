...
1. **Create the translation file:**
   * Navigate to the [src/languages/](src/languages/) folder.
   * Create a JSON file named after your language code (e.g., `it.json` or `es.json`).
   * Copy the structure of `pt-BR.json` or `en.json` and translate the values of the keys into your language.
2. **Add the language to the Dropdown Menu:**
   * Open [src/index.html](src/index.html).
   * Locate the `<ul class="dropdown-menu dropdown-menu-end shadow" aria-labelledby="btnLanguageSelector">` tag.
   * Add a new list item (`<li>`) invoking the `switchLanguage` function with your language code. For example:
     ```html
     <li><a class="dropdown-item py-2" href="#" onclick="switchLanguage('it'); return false;">Italiano (it)</a></li>
     ```
   * *Tip:* Keep the list ordered with Latin-alphabet languages alphabetically first, and other alphabets at the end.
3. **Translate Native Menus (Optional - for the Tauri desktop version):**
   ...
   * Open [src-tauri/src/lib.rs](src-tauri/src/lib.rs).
   * Locate the `create_menu` function and map your new language code in the `match lang` branches for `about_label`, `copyright`, `edit_label`, and `window_label`.
...
| Shortcut | Action |
| --- | --- |
| `Cmd + T` / `Ctrl + T` or `Cmd + N` / `Ctrl + N` | Create a New Document (New Tab) |
| `Cmd + O` / `Ctrl + O` | Open a Local File |
| `Cmd + S` / `Ctrl + S` | Save Changes to Active File |
| `Cmd + Shift + S` / `Ctrl + Shift + S` | Save As (Write to a New File) |
| `Cmd + W` / `Ctrl + W` | Close Active Tab (without quitting app) |
...
1. **翻訳ファイルの作成:**
   * [src/languages/](src/languages/) フォルダへ移動します。
   * 独自の言語コード（例: `it.json` や `es.json`）のJSONファイルを作成します。
   * `pt-BR.json` または `en.json` の構造をコピーし、各キーの値を翻訳します。
2. **ドロップダウンメニューへの追加:**
   * [src/index.html](src/index.html) ファイルを開きます。
   * `<ul class="dropdown-menu dropdown-menu-end shadow" aria-labelledby="btnLanguageSelector">` タグを見つけます。
   * 作成した言語コードを引数に指定して `switchLanguage` 関数を呼び出す `<li>` リスト項目を追加します。例:
     ```html
     <li><a class="dropdown-item py-2" href="#" onclick="switchLanguage('it'); return false;">Italiano (it)</a></li>
     ```
   * *ヒント:* リストは、ラテン文字（アルファベット）の言語を最初にアルファベット順で並べ、その後にラテン文字以外の言語（日本語、中国語など）を最後に配置してください。
3. **OS標準メニューの翻訳 (オプション - Tauriデスクトップ版用):**
   * デスクトップ版のOS標準メニュー（macOSの上部メニューバーなど）のラベルも翻訳できます。
   * [src-tauri/src/lib.rs](src-tauri/src/lib.rs) を開きます。
   * `create_menu` 関数を見つけ、`match lang` ブロックの `about_label`、`copyright`、`edit_label`、`window_label` に新しい言語用の分岐を追加します。
...
| ショートカット | アクション |
| --- | --- |
| `Cmd + T` / `Ctrl + T` または `Cmd + N` / `Ctrl + N` | 新規ドキュメント作成 (新しいタブ) |
| `Cmd + O` / `Ctrl + O` | ローカルファイルを開く |
| `Cmd + S` / `Ctrl + S` | 当前的ファイルへの変更を保存 |
| `Cmd + Shift + S` / `Ctrl + Shift + S` | 名前を付けて保存 (新しいファイルに書き込み) |
| `Cmd + W` / `Ctrl + W` | 当前のタブを閉じる (アプリは終了しない) |
...
python3 -m http.server 8000
```
その後、ブラウザで `http://localhost:8000/src/` を開きます。
